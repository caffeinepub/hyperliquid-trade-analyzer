  ...
  for (const value of firstRowValues) {
    ...
    for (const action of TRADING_ACTIONS) {
      if (normalized.includes(action)) {
        if (isDev)
          console.log(
            `[CSV Parser] Headerlose CSV erkannt: "${value}" enthält Trading-Aktion "${action}"`,
          );
        return true;
      }
    }
...
function analyzeColumnType(values: string[]): string {
  ...
  const symbolMatches = samples.filter((v) =>
    symbolPattern.test(v.trim()),
  ).length;
  ...
  const timestampMatches = samples.filter((v) =>
    timestampPattern.test(v.trim()),
  ).length;
  ...
  const dateTimeMatches = samples.filter((v) =>
    dateTimePattern.test(v.trim()),
  ).length;
  ...
  const numericMatches = samples.filter((v) =>
  ...
  if (numericMatches / samples.length > 0.8) {
    ...
    const avgValue =
      samples.reduce((sum, v) => {
        ...
        return sum + (Number.isNaN(num) ? 0 : Math.abs(num));
      }, 0) / samples.length;

    const _hasDecimals = samples.some(
    ...
    if (hasLargeValues) {
      ...
      const avgDecimals =
        decimalPlaces.reduce((a, b) => a + b, 0) / decimalPlaces.length;
...
function detectNumberFormat(values: string[]): "de" | "en" {
  ...
  if (samplesAnalyzed === 0) {
    if (isDev)
      console.log(
        "[CSV Parser] Keine Zahlenformate erkannt, verwende Standardformat: Englisch (Punkt als Dezimal)",
      );
    return "en";
  }
  ...
  if (isDev) {
    console.log(
      `[CSV Parser] Zahlenformat erkannt: ${format === "de" ? "Deutsch (Komma als Dezimaltrennzeichen)" : "Englisch (Punkt als Dezimaltrennzeichen)"}`,
    );
    console.log(
      `[CSV Parser] Analysierte Samples: ${samplesAnalyzed}, Komma als Dezimal: ${commaAsDecimalCount}, Punkt als Dezimal: ${dotAsDecimalCount}`,
    );
  }
...
function normalizeNumber(
  ...
  if (normalized === "" || normalized === "-") {
    if (isDev)
      console.log(
        `[CSV Parser] Ungültiger Wert für '${fieldName}': "${value}" -> 0`,
      );
    return 0;
  }
  ...
  try {
    if (commaCount === 0 && dotCount === 0) {
      ...
      if (lastCommaPos > lastDotPos) {
        ...
        result = Number.parseFloat(normalized.replace(/,/g, ""));
      }
      ...
      if (commaCount === 1) {
        ...
        if (detectedFormat === "de" || (parts[1] && parts[1].length <= 8)) {
          ...
          result = Number.parseFloat(normalized.replace(",", "."));
        } else {
    ...
    if (Number.isNaN(result) || !Number.isFinite(result)) {
      ...
      const englishAttempt = Number.parseFloat(normalized.replace(/,/g, ""));
      if (!Number.isNaN(englishAttempt) && Number.isFinite(englishAttempt)) {
        result = englishAttempt;
        if (isDev)
          console.log(
            `[CSV Parser] Fallback zu Englisch-Format für '${fieldName}': "${value}" -> ${result}`,
          );
      } else {
        const germanAttempt = Number.parseFloat(
          normalized.replace(/\./g, "").replace(",", "."),
        );
        if (!Number.isNaN(germanAttempt) && Number.isFinite(germanAttempt)) {
          result = germanAttempt;
          if (isDev)
            console.log(
              `[CSV Parser] Fallback zu Deutsch-Format für '${fieldName}': "${value}" -> ${result}`,
            );
        } else {
          if (isDev)
            console.warn(
              `[CSV Parser] Konnte Zahl nicht parsen für '${fieldName}': "${value}" -> 0`,
            );
          return 0;
        }
      }
    ...
    if (isDev && fieldName) {
      console.log(
        `[CSV Parser] Geparst '${fieldName}': "${value}" -> ${isNegative ? -result : result}`,
      );
    }
    ...
    if (isDev)
      console.error(
        `[CSV Parser] Fehler beim Parsen von '${fieldName}': "${value}"`,
        error,
      );
    return 0;
  }
...
function detectDelimiter(line: string): string {
  ...
  const hasHyperliquidColumns = /\b(coin|px|sz|ntl|fee|closedPnl|dir)\b/i.test(
    line,
  );
  ...
  if (hasHyperliquidColumns && commaCount > 0) {
    if (isDev)
      console.log(
        "[CSV Parser] Hyperliquid-Format erkannt, verwende Komma als Trennzeichen",
      );
    return ",";
  }
  ...
  if (isDev)
    console.log(
      `[CSV Parser] Erkanntes Trennzeichen: "${delimiter}" (Kommas: ${commaCount}, Semikolons: ${semicolonCount})`,
    );
  return delimiter;
}
...
function interpretTradeDirection(dirValue: string): {
  ...
  const isLiquidated =
    normalized.includes("liquidated") || normalized.includes("liquidiert");
  const isLong = normalized.includes("long");
  const isOpening =
    normalized.includes("open") || normalized.includes("öffnen");
  ...
  if (isDev) {
    console.log(
      `[CSV Parser] Trade-Richtung interpretiert: "${dirValue}" -> Liquidiert: ${isLiquidated}, Long: ${isLong}, Eröffnung: ${isOpening}`,
    );
  }
  ...
  try {
    ...
    if (parts.length >= 2) {
      ...
      if (dateParts.length === 3 && timeParts.length >= 2) {
        ...
        const seconds =
          timeParts.length > 2 ? Number.parseInt(timeParts[2], 10) : 0;
    ...
    if (!Number.isNaN(timestamp) && timestamp > 1000000000) {
      const date = new Date(
    ...
    if (isDev)
      console.warn(`[CSV Parser] Konnte Zeit nicht parsen: "${timeStr}"`);
    return "";
  } catch (error) {
    if (isDev)
      console.error(
        `[CSV Parser] Fehler beim Parsen der Zeit: "${timeStr}"`,
        error,
      );
    return "";
  }
...
function calculateLeverage(
  ...
  if (
    ...
    if (isDev) {
      console.log(
        "[CSV Parser] Hebel-Berechnung fehlgeschlagen: Negative Werte erkannt",
      );
    }
    return null;
  }
  ...
  if (safeEntryPrice < EPSILON || Math.abs(safePositionSize) < EPSILON) {
    if (isDev) {
      console.log(
        "[CSV Parser] Hebel-Berechnung fehlgeschlagen: Einstiegspreis oder Positionsgröße fehlt",
      );
    }
    return null;
  }
  ...
  if (safeMarginUsed > EPSILON) {
    ...
    if (
      calculatedLeverage >= 0.1 &&
      calculatedLeverage <= 1000 &&
      Number.isFinite(calculatedLeverage)
    ) {
      const rounded = roundToTwoDecimals(calculatedLeverage);
      if (isDev) {
        console.log(
          `[CSV Parser] Hebel berechnet (Primär): (${safeEntryPrice.toFixed(2)} * ${Math.abs(safePositionSize).toFixed(4)}) / ${safeMarginUsed.toFixed(2)} = ${rounded.toFixed(2)}x`,
        );
      }
      return rounded;
    }
  ...
  if (safeNotional > EPSILON && positionValue > EPSILON) {
    ...
    if (
      ...
      if (isDev) {
        console.log(
          `[CSV Parser] Hebel berechnet (Fallback 2): ${Math.abs(safeNotional).toFixed(2)} / (${safeEntryPrice.toFixed(2)} * ${Math.abs(safePositionSize).toFixed(4)}) = ${rounded.toFixed(2)}x`,
        );
      }
      return rounded;
    }
  ...
  if (safeFee > EPSILON) {
    ...
    if (
      ...
      if (isDev) {
        console.log(
          `[CSV Parser] Hebel berechnet (Fallback 3): (${safeEntryPrice.toFixed(2)} * ${Math.abs(safePositionSize).toFixed(4)}) / (${safeFee.toFixed(4)} + 0.0001) = ${rounded.toFixed(2)}x`,
        );
      }
      return rounded;
    }
  ...
  if (isDev) {
    console.log(
      "[CSV Parser] Hebel konnte nicht berechnet werden, gebe null zurück",
    );
    console.log(
      `[CSV Parser] Verfügbare Werte: entryPrice=${safeEntryPrice}, positionSize=${safePositionSize}, marginUsed=${safeMarginUsed}, notional=${safeNotional}, fee=${safeFee}`,
    );
  }
  return null;
}
...
export function parseCSV(
  csvText: string,
  fileName: string,
): { positions: TradePosition[]; summary: TradeSummary; fileName: string } {
  if (isDev)
    console.log("[CSV Parser] ========== STARTE CSV-PARSING ==========");
  ...
  const firstRowValues = splitCSVLine(lines[0], delimiter).map((v) =>
    v.trim().replace(/^"|"$/g, ""),
  );
  ...
  if (hasHeaders) {
    ...
    if (isDev)
      console.log(
        "[CSV Parser] CSV mit Headern erkannt. Rohe Spalten:",
        rawHeaders,
      );
    ...
    if (isDev)
      console.log(
        "[CSV Parser] Headerlose CSV erkannt. Führe dynamisches Spalten-Mapping durch...",
      );
  ...
  if (isDev)
    console.log(`[CSV Parser] Validierung: ${headers.length} Spalten erkannt`);
  ...
  for (let i = dataStartIndex; i < lines.length; i++) {
    ...
    if (values.length !== headers.length) {
      if (isDev)
        console.warn(
          `[CSV Parser] Zeile ${i + 1} hat ${values.length} Werte, erwartet ${headers.length}. Überspringe oder passe an.`,
        );
  ...
  if (isDev)
    console.log(`[CSV Parser] ${rows.length} Datenzeilen erfolgreich geparst`);
  ...
  if (isDev)
    console.log(
      "[CSV Parser] ========== STARTE POSITIONS-KONVERTIERUNG ==========",
    );

  const positions: TradePosition[] = rows.map((row, index) => {
    if (isDev)
      console.log(`[CSV Parser] --- Verarbeite Zeile ${index + 1} ---`);

    const timeStr = row.time || "";
    const timeISO = parseTimeToISO(timeStr);

    const directionValue = row.direction || row.dir || "";
    const directionInfo = directionValue
      ? interpretTradeDirection(directionValue)
      : null;
    ...
    const liquidationPrice =
      liquidationPriceValue > 0 ? liquidationPriceValue : undefined;
    ...
    const fee = normalizeNumber(row.fee || "0", "Fee", detectedFormat);
    ...
    const isLiquidated =
      directionInfo?.isLiquidated ||
      statusText.includes("liquidated") ||
      statusText.includes("liquidiert") ||
      ((statusText.includes("closed") || statusText.includes("geschlossen")) &&
        unrealizedPnl < -marginUsed * 0.9);

    if (isDev)
      console.log(
        `[CSV Parser] Liquidiert: ${isLiquidated} (aus dir: ${directionInfo?.isLiquidated}, aus status: ${statusText})`,
      );
    ...
    const _liquidationReason = isLiquidated
      ? directionInfo?.liquidationReason ||
        determineLiquidationReason(
          entryPrice,
          liquidationPrice,
          marginUsed,
          leverage || 1,
    ...
    const direction = directionInfo?.isLong
      ? TradeDirectionEnum.long_
      : TradeDirectionEnum.short_;
  ...
  if (isDev)
    console.log(
      `[CSV Parser] ${positions.length} Positionen erfolgreich konvertiert`,
    );
...
function calculateRiskLevel(
  ...
  const distanceToLiquidation =
    Math.abs(entryPrice - liquidationPrice) / entryPrice;
  const marginRatio = marginUsed > 0 ? unrealizedPnl / marginUsed : 0;

  if (distanceToLiquidation < 0.05 || marginRatio < -0.7) {
    return RiskLevel.extreme;
  }
  if (distanceToLiquidation < 0.1 || marginRatio < -0.5) {
    return RiskLevel.high;
  }
  if (distanceToLiquidation < 0.2 || marginRatio < -0.3) {
...
function determineLiquidationReason(
  entryPrice: number,
  liquidationPrice: number | undefined,
  marginUsed: number,
  leverage: number,
...
function calculateSummary(positions: TradePosition[]): TradeSummary {
  ...
  const liquidatedPositions = BigInt(
    positions.filter((p) => p.isLiquidated).length,
  );
  ...
  const totalMetalsPnl = metalPositions.reduce(
    (sum, p) => sum + p.realizedPnl + p.unrealizedPnl,
    0,
  );
