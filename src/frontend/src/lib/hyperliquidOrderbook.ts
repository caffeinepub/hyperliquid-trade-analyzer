...
export function parseOrderbookData(
  raw: RawOrderbookData | string,
): ParsedOrderbook {
  try {
    const data: RawOrderbookData =
    ...
    if (bids.some((b) => Number.isNaN(b.price) || Number.isNaN(b.size))) {
      throw new Error("Invalid bid data");
    }
    if (asks.some((a) => Number.isNaN(a.price) || Number.isNaN(a.size))) {
    ...
    throw new Error(
      `Failed to parse orderbook data: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
...
export function validateOrderbook(orderbook: ParsedOrderbook): boolean {
  ...
  if (
    ...
    Number.isNaN(orderbook.midPrice)
  )
    return false;
  return true;
}
