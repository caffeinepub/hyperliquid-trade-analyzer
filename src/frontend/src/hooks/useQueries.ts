...
import type { AnalysisSession, TradePosition, TradeSummary, AssetStats } from '../backend';
import { Principal } from '@icp-sdk/core/principal';
import { ExternalBlob, AssetCategoryEnum } from '../backend';
import { getAvailableAssets, getAvailableMetals, getAvailableStablecoins, calculateAssetStats, getMetalPositions, getStablecoinPositions, isMetal, isStablecoin } from '../lib/utils';
...
export function useStoreAnalysisSession() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const cryptoAssets = availableAssets.filter(asset => !isMetal(asset) && !isStablecoin(asset));
      ...
      const stablecoinStats: AssetStats[] = availableStablecoins.map(stablecoin => {
        const stats = calculateAssetStats(positions, stablecoin);
        const stablecoinPositions = positions.filter(p => p.symbol === stablecoin);
        return {
          symbol: stats.symbol,
          totalPnl: stats.totalPnl,
          averagePnl: stats.averagePnl,
...
export function useGetAssetStats(sessionId?: string, assetCategory?: AssetCategoryEnum | null) {
  const { actor, isFetching } = useActor();
