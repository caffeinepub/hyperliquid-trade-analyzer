...
import { CircleDollarSign, Coins, Filter, Search, X } from "lucide-react";
...
import {
  ...
  getMetalIcon,
  getNonMetalNonStablecoinAssets,
  getStablecoinIcon,
  isMetal,
  isStablecoin,
} from "../lib/tradeUtils";
...
export default function AssetSelector({
  ...
  const _availableAssets = getAvailableAssets(positions);
  ...
  const filteredCryptos = useMemo(
    () =>
      cryptos.filter((asset) =>
        asset.toLowerCase().includes(searchQuery.toLowerCase()),
      ),
    [cryptos, searchQuery],
  );

  const assetStats = selectedAsset
    ? calculateAssetStats(positions, selectedAsset)
    : null;
  const isMetalSelected = selectedAsset ? isMetal(selectedAsset) : false;
  const isStablecoinSelected = selectedAsset
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-end">
          <div className="flex-1 w-full sm:w-auto space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) =>
                onAssetChange(value === "all" ? null : value)
              }
            >
          ...
          {assetStats && (
            <div
              className={`flex-1 grid grid-cols-3 gap-4 p-4 rounded-lg ${
                isMetalSelected
            ...
            >
              <div>
                ...
                <p
                  className={`text-lg font-bold ${assetStats.totalPnl >= 0 ? "text-success" : "text-destructive"}`}
                >
              ...
              <div>
                ...
                <p
                  className={`text-lg font-bold ${assetStats.averagePnl >= 0 ? "text-success" : "text-destructive"}`}
                >
